...
/// Used by the implementation for int __mse_generate_indexes(mse_all_printings_cards_t *ret, mse_thread_pool_t *pool);
/// Which is in this unit

typedef struct mse_index_generator_state_t {
    mse_all_printings_cards_t *cards; // owned by the caller
    mse_thread_pool_t *pool; // owned by the caller
    sem_t semaphore;
    int ret;
