...
#define MSE_INDEX_COLOUR_NAME(colour_field, cmp_type) __add_card_to_##colour_field##_##cmp_type##_index
#define MSE_INDEX_COLOUR_NAME_IMPL(colour_field, cmp_type) __impl__add_card_to_##colour_field##_##cmp_type##_index

#define MSE_INDEX_FOR_COLOUR(colour_field, cmp_type) \
static int MSE_INDEX_COLOUR_NAME_IMPL(colour_field, cmp_type) \
    (avl_tree_node_t *cards, avl_tree_node_t **tree, mse_colour_flags_t colours) \
{ \
    if (cards == NULL) { \
        return 1; \
    } \
 ...
 \
    ASSERT(MSE_INDEX_COLOUR_NAME_IMPL(colour_field, cmp_type)(cards->l, tree, colours)); \
    ASSERT(MSE_INDEX_COLOUR_NAME_IMPL(colour_field, cmp_type)(cards->r, tree, colours)); \
    return 1; \
} \
...
{ \
    ...
    if (!MSE_INDEX_COLOUR_NAME_IMPL(colour_field, cmp_type)(state->cards->card_tree, \
        &state->cards->indexes.colour_index.colour_field##_##cmp_type.colour_indexes[colours], \
        colours)) { \
        state->ret = 0; \
    } \
